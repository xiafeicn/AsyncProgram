...
select top 10 *, NewID() as random from [V_QuestionJiaocaiSourceResult] where status=0 ").ToList();
        ...
        public List<QuestionXkw> AddQuestion(string html, int jiaocaiId, string subjectId, string sourceUrl)
        {
        ...
        public void WriteLog(string error, string error2)
        {
            using (var db = new XKWEntities2())
            {
                var log = new Log();
